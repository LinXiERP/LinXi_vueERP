<template id="CommodityInventory">
  <div>
    <el-row :gutter="21">
      <el-col :span="14">入库单详情表</el-col>
      <el-col :span="10">
        <el-form :inline="true" class="demo-form-inline">
          <el-form-item label="商品名称">
            <el-select v-model="SearchRecord.id" placeholder="所有">
              <el-option
          ...
          <el-form-item>
            <el-button type="primary" @click="SearchInRecord()">查询</el-button>
          </el-form-item>
        </el-form>
      </el-col>
      ...
      >
        ...
        <el-table-column prop="id" label="入库编号" width="180"></el-table-column>
        <el-table-column prop="commodityName" label="商品名称" width="180"></el-table-column>
        <el-table-column prop="nums" label="入库数量"></el-table-column>
        <el-table-column prop="operateTime" label="入库日期"></el-table-column>
        <el-table-column prop="staffName" label="经手人"></el-table-column>
        <el-table-column prop="sourceno" label="采购编号"></el-table-column>
    ...
    <el-dialog title="入库单详情" :visible.sync="centerDialogVisible" width="50%" center>
      <el-row>
        <el-col :span="8">
          <div class="grid-content bg-purple">入库编号：{{Commodityrecord.id}}</div>
        </el-col>
        <el-col :span="8">
          <div class="grid-content bg-purple-light">采购编号:{{Commodityrecord.sourceno}}</div>
        </el-col>
        <el-col :span="8">
          <div class="grid-content bg-purple-light">入库时间:{{Commodityrecord.operateTime}}</div>
        </el-col>
      </el-row>
      ...
      <el-row>
        ...
        <el-col :span="8">
          <div class="grid-content bg-purple-light">入库数量:{{Commodityrecord.nums}}</div>
        </el-col>
      </el-row>
      <el-divider></el-divider>
...
export default {
  ...
  methods: {
    GetIcCommoditRecord() {
      this.$axios.get(`/CommodityInventory/GetIcCommoditRecord`).then((res) => {
    ...
    SearchInRecord() {
      this.$axios
        .get("/CommodityInventory/SearchInRecord", {
          params: {
            id: this.SearchCommodity.id,
            sourceno: this.SearchCommodity.sourceno,
          },
        })
        .then((res) => {this.PuOrderInfos = res.data;
         this.CommodityRecords.forEach(element => {
          element.operateTime=element.operateTime.substring(0, 10);
        });
        });
