...
export default {
  ...
  methods: {
    BtnSearch() {
      ...
      this.$axios
        .get(
          `/IntercourseManagement/GetAllPurchasingSend?Slno=${this.formSearch.Slno}&OrderId=${this.formSearch.OrderId}`
        )
        .then((res) => {
    ...
    handleCurrentChange(val) {
      ...
      this.$axios
        .get(
          `/PurchasingManagement/GetAllPurchasingSend?pageIndex=${val}&no=${this.formSearch.no}&SearchName=${this.formSearch.SearchName}&CategoryName=${this.formSearch.CategoryName}`
        )
        .then((res) => {
          if (res.data.entity.length != 0) {
            res.data.entity.forEach((element) => {
    ...
    async loadinfo() {
      const { data: res } = await this.$axios.get(
        "/IntercourseManagement/GetAllPurchasingSend"
      );
      this.loading = false;
