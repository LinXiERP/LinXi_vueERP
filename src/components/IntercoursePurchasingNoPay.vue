...
export default {
  ...
  methods: {
    BtnSearch() {
      var that = this;
      this.$axios
        .get(`/IntercourseManagement/GetAllPuOrder?no=${this.formSearch.no}`)
        .then((res) => {
    ...
    handleCurrentChange(val) {
      ...
      this.$axios
        .get(
          `/PurchasingManagement/GetAllPuOrder?pageIndex=${val}&no=${this.formSearch.no}`
        )
        .then((res) => {
          if (res.data.entity.length != 0) {
            res.data.entity.forEach((element) => {
    ...
    async loadinfo() {
      const { data: res } = await this.$axios.get(
        "/IntercourseManagement/GetAllPuOrder"
      );
      this.loading = false;
